/**
 * Utilitários para validação de procedures e parâmetros
 */
export class ProcedureValidator {
   ...
   */
  static isValidProcedureCall(procedureCall: string): boolean {
   ...
   */
  static extractProcedureName(procedureCall: string): string | null {
    if (!this.isValidProcedureCall(procedureCall)) {
      return null;
    }
   ...
   */
  static isSafeProcedureCall(procedureCall: string): boolean {
    if (!this.isValidProcedureCall(procedureCall)) {
      return false;
    }
   ...
   */
  static sanitizeProcedureCall(procedureCall: string): string {
   ...
   */
  static validateExecutionConfig(config?: {
    timeout?: number;
    [key: string]: unknown;
  }): string[] {
    const errors: string[] = [];
    ...
    if (config.timeout !== undefined && !this.isValidTimeout(config.timeout)) {
      errors.push(
        "Timeout deve ser um número positivo até 300000ms (5 minutos)",
      );
    }
