...
import { ViaCepResponse, AddressData } from "./types/cep.types";
...
export class CepService {
   ...
   */
  private static normalizeCep(cep: string): string {
  ...
  static async fetchAddressByCep(cep: string): Promise<AddressData | null> {
    ...
    if (!this.validateCep(normalizedCep)) {
  ...
  static async searchAddresses(
    ...
    try {
      ...
      const response = await fetch(
        `${this.VIACEP_BASE_URL}/${uf}/${encodeURIComponent(city)}/${encodeURIComponent(street)}/json/`,
        {
          method: "GET",
          headers: {
            Accept: "application/json",
