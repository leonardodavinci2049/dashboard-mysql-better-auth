...
/**
 * Classe utilitária para formatação de respostas de procedures
 */
export class ResponseFormatter {
   ...
   */
  static formatProcedureResponse<T extends RowDataPacket = RowDataPacket>(
   ...
   * @returns Resposta formatada
   */
  static formatDataResponse<T extends RowDataPacket = RowDataPacket>(
    resultData: T[],
  ): GenericProcedureResponse<T[]> {
    return {
      success: true,
   ...
   * @returns Resposta formatada
   */
  static formatModifyResponse(resultData: {
    affectedRows: number;
    [key: string]: unknown;
   ...
   */
  static formatErrorResponse<T = unknown>(
    error: Error | string,
  ...
  /**
   * Método para formatar e exibir resultados de forma legível
   * @param response - Resposta da procedure
   * @returns String formatada para visualização
   */
  static formatForDisplay(response: GenericProcedureResponse<unknown>): string {
    const lines = [
   ...
   */
  static toCompactJSON(response: GenericProcedureResponse<unknown>): string {
    const compact = {
   ...
   */
  static extractData<T>(response: GenericProcedureResponse<T>): T | null {
